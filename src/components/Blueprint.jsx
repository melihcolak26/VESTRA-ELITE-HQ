...
const Blueprint = ({ onBack }) => {
  ...
  const methods = [
    { code: 'ENT-01', name: 'Shannon Entropy', type: 'Foundation Layer', diff: 2, time: '5', what: 'Verideki belirsizliği ölçerek objektif ağırlık temelleri atarsın.', emoji: '🎲', when: 'Veriler konuşsun istendiğinde.', icon: <Layers size={40} /> },
    { code: 'CRT-02', name: 'CRITIC Method', type: 'Foundation Layer', diff: 3, time: '10', what: 'Kriterler arası çatışmayı bularak bilgi tekrarını önlersin.', emoji: '⚖️', when: 'Çelişkili hedefler varsa.', icon: <Activity size={40} /> },
    { code: 'AHP-03', name: 'AHP Protocol', type: 'Foundation Layer', diff: 4, time: '15', what: 'Uzman görüşlerini matematiksel tutarlılıkla temele dökersin.', emoji: '🧠', when: 'Vizyon ve tecrübe odaklı kararlarda.', icon: <Brain size={40} /> },
    { code: 'TPS-04', name: 'TOPSIS Elite', type: 'Structure Layer', diff: 2, time: '5', what: 'İdeal çözüme en yakın noktayı geometrik olarak bulur.', emoji: '🎯', when: 'Genel seçim ve yatırım analizlerinde.', icon: <Target size={40} /> },
    { code: 'EDA-05', name: 'EDAS Robust', type: 'Structure Layer', diff: 3, time: '8', what: 'Ortalama performanstan sapma analiziyle sağlam yapılar kurar.', emoji: '📈', when: 'Volatilite ve uç değerler varsa.', icon: <Compass size={40} /> },
    { code: 'WAS-09', name: 'WASPAS Matrix', type: 'Structure Layer', diff: 5, time: '15', what: 'Hibrit modellerle %99.4 akademik hassasiyete ulaşır.', emoji: '💎', when: 'Maksimum teknik doğruluk gerektiğinde.', icon: <Gem size={40} /> }
  ];
    ...
    <div className="blueprint-paper fade-in">
      ...
      <nav style={{ padding: '25px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--bp-line)', background: 'rgba(2, 6, 23, 0.9)', position: 'sticky', top: 0, zIndex: 1000, backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span style={{ fontSize: '0.6rem', fontWeight: 800, color: 'var(--bp-primary)', letterSpacing: '0.3em' }}>KARAR MİMARİNİZ v10.0</span>
          </div>
        </div>
      ...
      <div className="container" style={{ maxWidth: '1250px', padding: '100px 20px' }}>
                ...
                <div key={i} className="blueprint-card" style={{ borderRadius: '30px', padding: '0', overflow: 'hidden' }}>
                   ...
                   <div style={{ padding: '40px' }}>
                      ...
                      <div style={{ background: 'rgba(255,255,255,0.03)', padding: '25px', border: '1px dashed var(--bp-line)', borderRadius: '20px' }}>
                        ...
                        <p className="technical" style={{ fontSize: '0.8rem', fontWeight: 900, color: 'var(--bp-primary)', marginBottom: '5px' }}>ÖRN:</p>
        ...
        <section style={{ textAlign: 'center' }}>
           <div style={{ padding: '120px 60px', borderTop: '1px solid var(--bp-line)', background: 'linear-gradient(to bottom, rgba(16,185,129,0.03), transparent)' }}>
              <Award size={80} color="var(--bp-primary)" style={{ margin: '0 auto 40px' }} />
              <h2 className="mono" style={{ fontSize: '4.5rem', fontWeight: 900, marginBottom: '30px' }}>İnşaya Hazır Mısınız?</h2>
              <p className="technical" style={{ fontSize: '1.2rem', color: '#94A3B8', marginBottom: '60px' }}>Hata payını matematiksel olarak sıfırladık. Kendi planınızı şimdi çizin.</p>
              <button onClick={onBack} className="btn-bp" style={{ padding: '25px 80px', fontSize: '1.5rem', borderRadius: '100px' }}>
                 ŞİMDİ BAŞLAT <ArrowRight size={32} />
              </button>
           </div>
        </section>
